...
function Admin_Emotions({ navigation, route }) {
  ...
  const gotoCRGR=(held)=>{
    navigation.navigate("Admin_CrGR_Report",{ heldid: held })
  }
    ...
    return heldclasses.map((classItem, index) => (
      <View style={styles.classContainer} key={index}>
        <View style={styles.classCard}>
          ...
          <View style={{marginTop:50}}><Button style={{backgroundColor:'#078345'}} onPress={()=>{gotoCRGR(classItem.held_id)}}><Text style={{color:'white'}}>CR & GR Report</Text></Button></View>
        </View>
      </View>
    ));
  };
