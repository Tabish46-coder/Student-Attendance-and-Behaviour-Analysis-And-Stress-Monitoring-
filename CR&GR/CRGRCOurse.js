...
const CRGR_Home = ({ navigation, route }) => {
    ...
    function handlebutton(code,sec){
        const registerationNo=route.params.user
        const roles=route.params.roleof
        const semesterNo='2024FM'
        console.log(registerationNo)
        navigation.navigate("Give_Feedback",{course:code,student:registerationNo,semester:semesterNo,Section:sec,role:roles})
    }

    const renderItem = ({ item }) => {
        const [courseCode, courseName,credihours,Section] = item.split(',').map((part) => part.trim());
        return (
            ...
            <View style={{height:30,width:30}}>
                <TouchableRipple style={{padding:5,width:'auto'}} onPress={()=>{handlebutton(courseCode,Section)}} rippleColor="#209920">
