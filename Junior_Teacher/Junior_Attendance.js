...
const Junior_Attendance = ({navigation,route}) => {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await fetch(
        `${callurl}jr-lecturer/assignments?jr_emp_no=${emp_no}&sems_no=2024FM`
      );
  ...
  const goto_attendance=(section,cource)=>{
    navigation.navigate('Teacher_Held_Classes',{ section_name: section,cource_code: cource,type:'UNSUPERVISED' });
  }

  const renderAssignmentCard = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={()=>{goto_attendance(item.Section,item.CourseCode)}}>
...
export default Junior_Attendance;