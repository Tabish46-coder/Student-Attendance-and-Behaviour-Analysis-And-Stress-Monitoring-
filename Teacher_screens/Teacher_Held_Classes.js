...
const Teacher_Held_Classes = ({navigation, route }) => {
    const { section_name,cource_code,type } = route.params;
    console.log(cource_code)
    ...
    useEffect(() => {
        const fetchHeldClasses = async () => {
            try {
                const response = await fetch(`${callurl}admin/fetchHeldClasses?section_name=${section_name}&course_id=${cource_code}&check=${type}`);
                if (!response.ok) {
    ...
    const gotoheldclassesdetails=(heldids)=>{
            ...
            navigation.navigate("Teacher_Held_Classes_Detail",{held_id:heldids})
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Held Classes for {section_name}</Text>
            <FlatList
                data={heldClasses}
...
export default Teacher_Held_Classes;